 ...
 */
public class BlogContentFetcher {
     ...
     * If the content is less than 200 words, summary is null.
     * @param path path of root
     * @param url file url
     * @return [summary, content]
     * @throws BlogContentNotFetchedException error
     */
    public static String[] fetch(String path, String url) throws BlogContentNotFetchedException {
        try {
    ...
    /**
     * Find the index of nth occurrence of space in a string, return -1 if not found.
     ...
     * It is a recursive method that helps keep the html format while removing things.
     * @param e element
    ...
    private static void removeAdditionalWords(Node e, int maxWords) {
        ...
        final int originalSize = children.size();
        if (originalSize == 0) {
            // base case: cannot deal with children any more, needs to actually remove words.
        ...
        int removeChildrenStartingAtIndex = children.size(); // initialize to length, assuming no removal of children
        // A loop to check starting from which child node, word limit has been exceeded.
        for (int i = 0; i < originalSize; i++) {
            ...
            if (wordLimitLeft == childWordCount) {
                ...
                // just enough, stop here. No need to waste resources on the recursive method.
                break;
        ...
        // remove nodes that comes after the node that just exceeds the word limit
        for (int i = removeChildrenStartingAtIndex; i < originalSize; i++) {
            children.get(removeChildrenStartingAtIndex).remove();
            /*
                Due to the implementation of Jsoup lib, it only removes the element from doc tree.
    ...
    /**
     * The summary of the article is defined to be the first 200 words or the entire article within 200 words.
     * This method calls the recursive private method removeAdditionalWords if word limit has been exceeded.
